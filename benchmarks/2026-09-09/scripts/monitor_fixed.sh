...
while :; do
  ...
  # 2026-09-08: the daemon no longer prints '[dl] heartbeat'; the applied height is
  # on the '[dlc] ==' summary and the '[utxo_live] catchup progress' line.
  hb=$(grep -E '\[dlc\] == elapsed' console.log | tail -1 | sed 's/.*\[dlc\] == //')
  ap_h=$(echo "$hb" | grep -oE 'applied=[0-9]+' | cut -d= -f2)
  cp_h=$(grep -oE 'catchup progress: height=[0-9]+' console.log | tail -1 | grep -oE '[0-9]+$')
  ours=$(printf '%s\n' "${ap_h:-0}" "${cp_h:-0}" | sort -n | tail -1); [ "$ours" = 0 ] && ours=
  ...
  for m in 'cookie authentication' 'utxo_live\] init' 'coinstats\] adopted' 'keep-up' 'caught up'; do
    l=$(grep -m1 -E "$m" console.log | cut -c1-140); [ -n "$l" ] && ! grep -qF "PHASE first '$m'" "$PH" && ph "MON: PHASE first '$m': $l elapsed=$(( $(date +%s)-T0 ))s"   # 2026-09-09: the marker check matched nothing, so this repeated every tick
  done
  if [ -n "$ours" ]; then
