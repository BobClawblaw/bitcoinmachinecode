...
| Full wallet-core + bitcoin-cli/RPC surface (getnewaddress, sendtoaddress, signrawtransaction, getbalance, createrawtransaction, etc.) | DONE (command layer + network layer client): getnewaddress/getrawchangeaddress/getaddressinfo/validateaddress, listunspent/gettxout, decoderawtransaction, signrawtransactionwithkey, sendtoaddress/getbalance, createrawtx/sign/send all in wallet_core.c + wallet_cli (tests test_wrpc_* + test_send + test_rpc_json + test_rpc_transport all green). bitcoin-cli network layer DONE (t_8e5be37f): JSON-RPC 2.0 framing + HTTP POST over a local socket with Basic auth (rpc_net.c), Core-bit-exact UniValue write(2) renderer (rpc_json.c), shared dispatch/render (rpc_commands.c), and daemon/bitcoin_cli binary -- verified end-to-end via test_rpc_transport over a real loopback HTTP socket (string results raw, objects/arrays via write(2), RPC+transport errors exact, 19 checks). REMAINS: the production HTTP JSON-RPC server endpoint (child card t_0ca5d72e, dispatches through the same rpc_dispatch) | this batch + post-batch |
