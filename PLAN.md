...
| Consensus bit-exactness on mainnet edge cases (BIP16/30, 2-of-3 P2SH, height-gated rules) | RUNNING — differential consensus harness BUILT (`validation/consensus_diff.py` + `asm/tests/consensus_shim`): feeds the same real mainnet block/tx bytes to the ASM consensus stack and Core's RPC and compares verdicts (accept path: ASM cons_verify + block_hash must match Core's active chain; reject path: mutated real blocks through both `cons_verify` and `submitblock`; plus per-tx BIP141 txid vs Core). Zero divergences across BIP16/34/SegWit/Taproot/recent epochs so far. Remaining edge-case sweeps feed downstream t_26f50e1b | this card t_67a30097 |
