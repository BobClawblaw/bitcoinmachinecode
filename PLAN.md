...
| Consensus bit-exactness on mainnet edge cases (BIP16/30, 2-of-3 P2SH, height-gated rules) | RUNNING — base differential harness BUILT (`validation/consensus_diff.py` + `asm/tests/consensus_shim`, t_67a30097, zero block-level divergences). THIS card (t_26f50e1b) added the **height-gated block-level rules** and drove them to zero divergence vs Core: new `asm/bitcoin_sigops.asm` (Core-exact `script_sigops` accurate/inaccurate + `tx_legacy_sigops` incl. SegWit marker skip) + shim `SIGOPS`/`DUPTX`; new `validation/sigops_diff.py` builds consensus-valid regtest blocks with tuned sigop counts and feeds identical bytes to Core `submitblock` + ASM — **core sigop block-limit rule (`MAX_BLOCK_SIGOPS=20000`, `bad-blk-sigops`) verified zero divergence** (below-limit not sigop-flagged, above-limit rejected `bad-blk-sigops` by both). Swept 11+ real mainnet epochs (genesis..962580) → sane counts, no crashes. `tests/test_sigops` 13 checks green. Remaining script-level BIP16/P2SH + BIP30-chain-context sweeps feed downstream | this card t_26f50e1b |
