...
| Taproot / segwit v1 validation (BIP341/340) | DONE — BIP340 Schnorr verify (19/19 official vectors) + BIP341 taproot helpers (x-only tweak w/ parity, tagged-hash tapleaf merkle root, control block, script version, annex/ext rules) in secp256k1_schnorr.asm + secp256k1_taproot.asm; bech32m P2TR address <-> scriptPubKey (BIP350) in wallet_core.c/bech32.asm; end-to-end BIP341/342 spend validation in bitcoin_taproot_sighash.c (SigMsg serialization byte-exact vs official Core wallet-test-vectors, key-path verify, script-path CHECKSIG/CHECKSIGADD verify, checksig_fn wired into the ASM script interpreter for live tapscript spends). test_taproot_sighash 42 checks green; oracle gen_taproot_vectors.py independently re-derives the vectors. REMAINS: full mempool-acceptance parity + witness-v0 in downstream t_84752b9b | this card t_93b2695f |
| Full wallet-core + bitcoin-cli/RPC surface (getnewaddress, sendtoaddress, signrawtransaction, getbalance, createrawtransaction, etc.) | DONE (command layer): getnewaddress/getrawchangeaddress/getaddressinfo/validateaddress, listunspent/gettxout, decoderawtransaction, signrawtransactionwithkey, sendtoaddress/getbalance, createrawtx/sign/send all in wallet_core.c + wallet_cli (tests test_wrpc_* + test_send all green). REMAINS: the HTTP RPC transport / a real bitcoin-cli network layer (commands exist; wire-up not done) | this batch + post-batch |
