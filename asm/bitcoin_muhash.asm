...
;
; num3072_mul is a CPU dispatcher over two bit-identical bodies (see it for
; the probe and the seam). Everything from num3072_mul_adx down is exported
...
; every `call`. Computed, not eyeballed. The one exception is num3072_mul_adx,
; which uses no callee-saved register and pushes only rbp: one push leaves
; RSP at 0 mod 16, so ITS reservation is 0 mod 16 (stated at the function).
; ============================================================================
default rel
...
;
;   Dispatcher. Two bodies compute this product, bit-identical to each other
;   and to Core's Num3072::Multiply (the identity argument is written out at
;   num3072_mul_adx below; tests/test_muhash_mul_diff.c holds them to it):
...
;                             ran before the ADX body existed.
;   Lazy one-time CPUID probe, cached in num3072_mul_path as a TRI-state
;   exactly like sha256.asm's shani_ready: 0 = not probed yet, 1 = ADX body,
;   2 = generic (cached too, so an absent configuration never re-probes).
;   The probe is CPUID.(EAX=7,ECX=0):EBX bit 8 (BMI2) and bit 19 (ADX),
;   gated on the max basic leaf -- the same leaf-7 shape sha256_cpu_has_sha
;   has had since CRY-1, where a leaf-1 probe once selected SHA-NI on CPUs
;   without it and SIGILL'd on the first hash. Tail-jumps only: rdi/rsi are
;   untouched, so the body sees the caller's arguments.
; ============================================================================
global num3072_mul
num3072_mul:
...
;   takes one body. On a host with ADX -- the gate box -- the generic body,
;   which is what a CPU without ADX depends on, would otherwise never execute
;   under `make test`; on a host without it the ADX body would not. With the
;   seam tests/test_muhash runs Core's vectors down every body the CPU can
;   run, and tests/test_muhash_mul_diff compares the bodies to each other.
;   Nothing in the daemon calls it; 0/unprobed is what production sees.
; ============================================================================
...
;   same helpers, so every value that reaches a[] is the one the generic
;   body writes. tests/test_muhash_mul_diff.c holds the two bodies to that
;   on 100,000 random pairs, the edges around the modulus, and a chained
;   accumulator; tests/test_muhash runs Core's own vectors down both.
;
...
;   addressed from rsp.
;   Frame 0x310 = 784 bytes: [rsp+0] a, [rsp+8] the top carry (Core's c0),
;   [rsp+16 .. +783] P[96]. Parity: entry 8 mod 16, one push -> 0 mod 16,
;   784 is 0 mod 16 -> RSP is 0 mod 16 at each of the three calls below.
...
section .bss
align 8
; num3072_mul's cached CPUID verdict: 0 unprobed, 1 ADX body, 2 generic.
