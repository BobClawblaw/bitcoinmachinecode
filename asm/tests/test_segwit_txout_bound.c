/* test_segwit_txout_bound.c -- incident #20 regression: sw_ser_txout() wrote
...
int main(int argc, char** argv){
    ...
    if (!dump){
        printf("== BIP143 sighash vs Core over CTxOut serialization (incident #20) ==\n");
     ...
     * identically. The plain build is the one that matters here anyway: gcc's
     * default -fstack-protector-strong turns the old overrun into a
     * deterministic "stack smashing detected" abort at 590 bytes. */
