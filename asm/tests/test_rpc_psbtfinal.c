...
int main(void){
    /* ---- taproot SCRIPT PATH: finalize to exactly what Core finalized -------
     * BIP371 PSBT_IN_TAP_SCRIPT_SIG (0x14) + PSBT_IN_TAP_LEAF_SCRIPT (0x15).
     * The register carried this as a GAP: "key path only, for every key type;
     * there is no 0x14/0x15 handling at all". Deferred because doing it
     * without Core-generated fixtures means guessing at the layout -- so the
     * fixture in psbt_tapscript_vec.h was generated by asking Core: a regtest
     * wallet whose descriptor is tr(NUMS, {pk(A), pk(B)}), which can ONLY be
     * spent through a leaf, signed by Core with finalize=false, then
     * finalized by Core. testmempoolaccept on the result says allowed.
     *
     * Watched to fail first: with the 0x14/0x15 arm removed from fin_build,
     * finalizepsbt returns complete=false and no hex at all. */
    {
        char pj[4096];
        snprintf(pj, sizeof pj, "[\"%s\"]", PSBT_SIGNED_B64);
        long ec2 = 0; const char* em2 = NULL;
        rj_val* r = call("finalizepsbt", pj, &ec2, &em2);
        ck("taproot script path: finalizepsbt succeeds", r != NULL);
        if (r){
            rj_val* hx = rj_obj_get(r, "hex");
            rj_val* cp = rj_obj_get(r, "complete");
            ck("taproot script path: reports complete", cp && cp->str && cp->str[0] == '1');
            ck("taproot script path: returns a raw transaction", hx && hx->typ == RJ_STR && hx->str);
            if (hx && hx->str){
                int same = !strcmp(hx->str, FINAL_TX_HEX);
                ck("taproot script path: byte-identical to what CORE finalized", same);
                if (!same){
                    printf("      ours: %.80s\n", hx->str);
                    printf("      core: %.80s\n", FINAL_TX_HEX);
                }
