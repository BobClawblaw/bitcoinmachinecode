...
int main(void){
    ...
    FILE* f=fopen("index.dat","rb"); fread(idx,1,total*48,f); fclose(f);
    ...
    for(long h=0;h<total;h++){
        ...
        if(pos<minpos)minpos=pos; if(pos+8+sz>maxpos)maxpos=pos+8+sz;
