; ============================================================================
; tests/fe_ref.asm -- FROZEN pre-PERF_SCOPE-5 copy of asm/secp256k1_fe.asm
;   (main @ cb20051), every exported symbol renamed with a _ref suffix:
;   fe_add_ref / fe_sub_ref / fe_mul_ref / fe_sqr_ref / fe_inv_ref.
;
;   Test-only differential reference for the field-representation work. It is
;   the 4x64 immediate-reduction implementation that ran the live replay to
;   height ~575,000, INCLUDING the two lost-carry fixes of 2026-08-21
;   (sc_mul MULACC and fe_mul fold 2, see tests/test_mul_carry_regression.c).
;   DO NOT EDIT -- its whole value is that it does not change. Not linked into
;   any daemon or tool.
; ============================================================================
; ============================================================================
...
;   void fe_add_ref(u64 r[4], const u64 a[4], const u64 b[4])   r = (a+b) mod p
;   void fe_sub_ref(u64 r[4], const u64 a[4], const u64 b[4])   r = (a-b) mod p
;   void fe_mul_ref(u64 r[4], const u64 a[4], const u64 b[4])   r = (a*b) mod p
;
...
; The exponent used by fe_inv_ref (Fermat a^(p-2)). Kept in this FIRST .rodata
; block: splitting .rodata across two blocks in the source caused a link/rip-
; relative resolution bug that made fe_inv_ref read the wrong exponent. P_LIMBS and
...
.zero:
    ...
    ; already proven correct elsewhere in this file, e.g. fe_add_ref) -- so the
    ...
    ; 2^256 == C (mod p), so fe_inv_ref(p - k) was wrong for small k (found by
...
; ============================================================================
; void fe_sqr_ref(u64 r[4], const u64 a[4])
;   r = (a * a) mod p. Squaring is the same modular multiply with both
;   operands equal, so we simply forward to fe_mul_ref. No separate code needed.
; ============================================================================
global fe_sqr_ref
fe_sqr_ref:
    mov rdx, rsi            ; a is both multiplicands: rd/... rdx = a (2nd op)
    jmp fe_mul_ref              ; fe_mul_ref(r, a, a)  (rdi=r, rsi=a already set)
...
;       result := fe_sqr_ref(result)
;       if that exponent bit is set:  result := fe_mul_ref(result, a)
;   r := result
;
; REGISTER plan (values that must survive the many fe_mul_ref calls live in
; callee-saved registers, which fe_mul_ref preserves: rbx, r12-r15; rbp too):
...
fe_inv_ref:
    ...
    ; survives every fe_mul_ref call). rdi is caller-saved and is clobbered by the
