 ...
 * and (2026-08-19, Stage D) that height/is_coinbase survive both a full WAL
 * replay and a checkpoint+tail-replay restart -- the whole reason those
 * fields exist is the 100-block coinbase maturity check, so losing them
 * across a crash-recovery path would silently defeat it.
 *
...
int main(void) {
    ...
    /* A0 is a coinbase output at height 100 -- exercised end to end below. */
    ck("store_put A0 new (coinbase, h=100)", utxo_store_put(&st, g_ux, tA, 0, 50000ULL, 100, 1, scrA, 25), 1);
    ck("store_put B7 new (normal, h=200)", utxo_store_put(&st, g_ux, tB, 7, 1234567ULL, 200, 0, scrB, 22), 1);
    ck("store_put C0 new (normal, h=201)", utxo_store_put(&st, g_ux, tC, 0, 999ULL, 201, 0, scrC, 5), 1);
    ck("count 3", utxo_store_count(&st, g_ux), 3);

    unsigned long long v; unsigned long h, cb, sl; const unsigned char* s;
    ck("get A0", utxo_store_get(&st, g_ux, tA, 0, &v, &h, &cb, &s, &sl), 1);
    ck("get A0 value", v, 50000ULL);
    ck("get A0 height", h, 100);
    ck("get A0 is_coinbase", cb, 1);
    ...
    ck("get B7 miss (spent)", utxo_store_get(&st, g_ux, tB, 7, &v, &h, &cb, &s, &sl), 0);
    ck("double spend miss", utxo_store_del(&st, g_ux, tB, 7), 0);

    /* dedup: putting A0 again logs a PUSH but leaves table unchanged */
    ck("store_put A0 dup", utxo_store_put(&st, g_ux, tA, 0, 9ULL, 999, 0, scrA, 25), 0);
    ...
    /* ---------- phase 2: RESTART-RESUME (full WAL, no checkpoint) ---------- */
    {
        ...
        ck("reload get A0", utxo_store_get(&st2, ux2, tA, 0, &v, &h, &cb, &s, &sl), 1);
        ck("reload A0 value", v, 50000ULL);
        ck("reload A0 height (survives WAL replay)", h, 100);
        ck("reload A0 is_coinbase (survives WAL replay)", cb, 1);
        ck("reload A0 slen", sl, 25);
        ck("reload A0 script", memcmp(s, scrA, 25) == 0, 1);
        ck("reload C0", utxo_store_get(&st2, ux2, tC, 0, &v, &h, &cb, &s, &sl), 1);
        ck("reload C0 height", h, 201);
        ck("reload C0 is_coinbase false", cb, 0);
        ck("reload B7 absent", utxo_store_get(&st2, ux2, tB, 7, &v, &h, &cb, &s, &sl), 0);
    ...
    /* ---------- phase 3: checkpoint (sync) + crash-tail restart ---------- */
    {
        ...
        ck("store_put E0 (tail, h=301)", utxo_store_put(&st, g_ux, tE, 0, 5555ULL, 301, 0, scrE, 10), 1);
        ...
        ck("reload3 C0", utxo_store_get(&st3, ux3, tC, 0, &v, &h, &cb, &s, &sl), 1);
        ck("reload3 C0 value", v, 999ULL);
        ck("reload3 C0 height (survives checkpoint)", h, 201);
        ck("reload3 D0", utxo_store_get(&st3, ux3, tD, 0, &v, &h, &cb, &s, &sl), 1);
        ck("reload3 D0 value", v, 7777ULL);
        ck("reload3 D0 height (survives checkpoint)", h, 300);
        ck("reload3 D0 is_coinbase (survives checkpoint)", cb, 1);
        ck("reload3 D0 script", memcmp(s, scrD, 8) == 0, 1);
        ck("reload3 E0", utxo_store_get(&st3, ux3, tE, 0, &v, &h, &cb, &s, &sl), 1);
        ck("reload3 E0 value", v, 5555ULL);
        ck("reload3 E0 height (survives tail replay)", h, 301);
        ck("reload3 E0 is_coinbase false (survives tail replay)", cb, 0);
        ck("reload3 A0 spent (from tail DEL)", utxo_store_get(&st3, ux3, tA, 0, &v, &h, &cb, &s, &sl), 0);
        ck("reload3 B7 spent", utxo_store_get(&st3, ux3, tB, 7, &v, &h, &cb, &s, &sl), 0);
        /* outpoints that never existed stay absent */
        unsigned char tZ[32]; make_txid(tZ, 0x99, 9);
        ck("reload3 tZ absent", utxo_store_get(&st3, ux3, tZ, 0, &v, &h, &cb, &s, &sl), 0);
     ...
     * PUSH_HDR is now 59 (was 54): magic(4)+op(1)+pad(3)+txid(32)+index(4)
     * +value(8)+height(4)+is_coinbase(1)+slen(2). Checkpoint records are now
     * 51 bytes (was 46): txid(32)+index(4)+value(8)+height(4)+is_coinbase(1)
     * +slen(2) -- the same fixed shape as the WAL PUSH body, minus its
     * magic/op/pad framing (see bitcoin_utxo_store.asm's CKPT_REC). */
    {
        /* utxo.dat: replay op records. First record = PUSH A0 (coinbase, h=100).
         * magic 'UTXO' (little-endian 0x5554584F), op=1, txid tA, index 0,
         * value 50000, height 100, is_coinbase 1, slen 25, script. */
        FILE* f = fopen("utxo.dat", "rb");
        ckm("utxo.dat exists", f != NULL);
        unsigned char hdr[59]; size_t n = fread(hdr, 1, 59, f);
        ck("rec0 hdr 59B read", n, 59);
        unsigned magic; memcpy(&magic, hdr, 4);
        ck("rec0 magic UTXO", magic, 0x5554584fUL);
        ck("rec0 op PUSH", hdr[4], 1);
        ckm("rec0 txid == tA", memcmp(hdr + 8, tA, 32) == 0);
        unsigned idx; memcpy(&idx, hdr + 40, 4); ck("rec0 index", idx, 0);
        unsigned long long val; memcpy(&val, hdr + 44, 8); ck("rec0 value", val, 50000ULL);
        unsigned height; memcpy(&height, hdr + 52, 4); ck("rec0 height", height, 100);
        ck("rec0 is_coinbase", hdr[56], 1);
        unsigned slen2; memcpy(&slen2, hdr + 57, 2); ck("rec0 slen", slen2, 25);
        ...
        unsigned char irec[51]; n = fread(irec, 1, 51, g);
        ck("idx rec 51B read", n, 51);
        /* checkpoint order follows table slot order, not insertion order, so
         * assert only on the shape (fields land where CKPT_REC says), not on
         * which specific outpoint this first record is. */
        memcpy(&slen2, irec + 49, 2);
        ckm("idx rec slen field in plausible range", slen2 <= 65535);
        fclose(g);
