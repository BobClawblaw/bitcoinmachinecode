...
extern long utxo_lsm_put(void* lst, void* u, const u8 txid[32], unsigned index,
                          unsigned long long value, const u8* script, unsigned slen);
...
int main(void){
    ...
    cki("put A0 (simulated prior-block output)",
        utxo_lsm_put(&lst, g_ux, txidA, 0, 123456789ULL, scrA, sizeof scrA), 1);
...

    {
        ...
        cki("A0 now missing from live set (spent)", utxo_lsm_get(&lst, g_ux, txidA, 0, &v, &s, &sl), 0);
    }
    ...
    cki("put B0", utxo_lsm_put(&lst, g_ux, txidB, 0, 42ULL, scrB, sizeof scrB), 1);
     ...
     * ============================================================ */
    {
        ...
        for (long h = 0; h <= 250; h++){
            ...
            cki("seed dummy undo record", undo_append_record(h, dummy_txid, 0, 1, dummy_script, 3), 1);
        }
