...
int main(void){
  ...
  for(unsigned f=0; f<COV_N; f++){ const cov_fixture_t* F=&COV_FIXTURES[f];
    ...
    /* KNOWN BUG (found by this test 2026-08-22, reported, not yet fixed):
     * a real tapscript CSV spend Core accepted at 806500 is REJECTED with
     * "p2tr tapscript execution failed". Root cause localised to the taproot
     * driver: bitcoin_taproot_sighash.c's script_eval setup memsets the state
     * then never sets st.flags / st.tx_locktime / st.in_sequence / st.tx_version
     * (all left 0). Gated to expect the current reject so this fixture is
     * permanent and the suite stays green; flip to (r==1) when fixed. */
    if(strcmp(F->name,"tapscript_csv")==0){
      snprintf(nm,sizeof nm,"%s [KNOWN BUG: rejected today; see comment]",F->note);
      ck(nm, r==0);
      if(r==1) printf("       (tapscript CSV now PASSES -- remove this gate)\n");
      continue;
    }
    ...
    if(F->xfail_witcap){
      /* Expected to reject on current code (TXV_MAX_WIT_ITEMS=8) -- but only
       * for THAT reason. Any other reject, or an accept, is a surprise.
       * Flip to (r==1) once branch wit-items raises the cap. */
      int gated = (r==0) && reason && strstr(reason,"witness items")!=NULL;
      snprintf(nm,sizeof nm,"%s [gated: rejects until wit-items raises the cap]",F->note);
      ck(nm, gated || r==1);
      if(!(gated||r==1)) printf("       UNEXPECTED reason: %s (r=%d)\n",reason,r);
    } else {
      ck(nm, r==1);
