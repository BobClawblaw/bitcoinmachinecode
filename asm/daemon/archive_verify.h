...
/* Point a store handle's append cursor at the newest blk file, so an append
 * can never land below the archive frontier and break the layout. Call it
 * after store_reload on any handle that will append. */
void archive_store_frontier(void* st);
