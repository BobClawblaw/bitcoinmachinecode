...
/* signet (default challenge): display 00000008819873e9...ee1ef6 reversed.
 * MISSING until 2026-08-31: signet support postdates this header, and the
 * live apply wrote signet's genesis coinbase into the set -- found as
 * "one extra output, exactly 50 BTC" against the Core oracle's
 * gettxoutsetinfo at an identical tip. */
static const unsigned char BMC_SIGNET_GENESIS[32] = {
...
static inline int bmc_is_genesis_block(long height, const unsigned char blk_hash[32]){
    return height == 0 &&
           (memcmp(blk_hash, BMC_MAINNET_GENESIS,  32) == 0 ||
            ...
            memcmp(blk_hash, BMC_SIGNET_GENESIS,   32) == 0 ||
