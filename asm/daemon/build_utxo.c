...
typedef void (*input_cb)(void* ctx, const u8 txid[32], u32 index);
typedef void (*output_cb)(void* ctx, u32 out_index, u64 value, const u8* script, u32 slen);
...
static int walk_tx_io(const u8* tx, const u8* end, void* ctx, input_cb icb, output_cb ocb,
    ...
    u64 consumed;
    u64 n_in = read_varint(p, end, &consumed); if(!consumed) return 0; p += consumed;
    *out_nin = n_in;
    for (u64 i=0;i<n_in;i++){
        ...
        p += 36;
        u64 slen = read_varint(p, end, &consumed); if(!consumed) return 0; p += consumed;
        if ((u64)(end - p) < slen + 4) return 0;
        p += slen + 4; /* script + sequence */
    }
    u64 n_out = read_varint(p, end, &consumed); if(!consumed) return 0; p += consumed;
    *out_nout = n_out;
