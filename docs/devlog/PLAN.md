...
| mempool policy / RBF / fee | DONE, and now LIVE-WIRED into the P2P daemon (2026-08-18) — `bitcoin_serve.asm` `.do_tx` runs every inbound tx through `mpool_policy_add` + `txval_modern` (via new `daemon/tx_accept.c`, a per-connection read-only LSM UTXO snapshot) before relay/storage, replacing the previous zero-validation accept. `tests/test_tx_accept_e2e.c` proves the real pipeline end-to-end. See LOG.md 2026-08-17/18 entry. Remaining: soak test against live real peers | bitcoin_mempool_policy.c + daemon/tx_accept.c |
...
| Persistent UTXO to disk | DONE, and REPLACED by a from-scratch LSM-tree (2026-08-17/18) — `bitcoin_utxo_store.asm`'s single pre-sized hash table write-amplified ~13x during a full-archive replay (giant sparse structure taking scattered writes regardless of live-set size at that point). `bitcoin_utxo_lsm.asm`: bounded memtable (reuses `bitcoin_utxo.asm` unchanged) + per-generation WAL + sorted Bloom-filtered immutable runs + streaming k-way-merge compaction, mirroring Core's LevelDB-behind-bounded-cache design. Full-archive replay rate stays flat instead of collapsing. Now the live daemon's own UTXO store too (`daemon/utxo_live.c`, single-writer download worker + persisted-applied-height catch-up); `bitcoin_utxo_store.asm` itself untouched, still used as the LSM's own per-generation WAL engine. See LOG.md 2026-08-17/18 entry for the full bug/fix history | bitcoin_utxo_lsm.asm + daemon/utxo_live.c |
| sighash_all real-spend end-to-end | DONE | test_e2e_sighash.c (wallet CLI signs -> whole-tx validator accepts)
| Full script interpreter (all opcodes incl. tapscript/BIP342) | DONE (EvalScript core + tapscript) — all BASE opcodes (flow/stack/splice/bitwise/arithmetic incl. 32/64-bit ScriptNum, crypto SHA256/HASH160/HASH256/RIPEMD160, CODESEPARATOR, disabled/reserved) + tapscript/BIP342 (OP_SUCCESSx pre-scan, cleanstack/empty-stack, MINIMALIF-consensus, CHECKSIGVERIFY-valid (schnorr, like CHECKSIG; incident #16 fix), CHECKMULTISIG->TAPSCRIPT_CHECKMULTISIG, CHECKSIGADD gating) in bitcoin_interp.asm. Verified 67/67 BASE vectors byte-for-byte vs Core script_tests.json (tests/script_tests_diff.py) + tests/test_tapscript_interp.c (24 checks green). REMAINS for downstream tasks: wiring the signature callback (checksig_fn/sighash+ecdsa/schnorr) for live OP_CHECKSIG/CHECKMULTISIG/CHECKSIGADD spends (taproot t_93b2695f) and P2SH redeemScript VerifyScript wrapper | this card t_61f61ec5 |
