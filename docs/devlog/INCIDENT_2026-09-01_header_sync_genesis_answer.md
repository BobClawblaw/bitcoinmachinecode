...
2. **Single-hash locator.** `dlc_headers` sends only the mirror's tip hash. Any peer that does not know that hash (an IBD peer, a peer behind the tip, a peer on another chain) legitimately answers from its genesis. Core sends an exponential locator so the reply starts at the last common block. **Fixed in `u`:** the boot fetch is now in C with an exponential locator and per-page checks (`dlc_fetch_headers`); the asm `node_ibd_headers` — whose return value also turned out not to report what it appended — is no longer used at boot.
...
| 6 | The boot header fetch is C: exponential locator, every page checked (must extend a header we hold; overlap identical; links; no txs; cap) before it is stored; the asm `node_ibd_headers` is no longer used at boot. | `dlc_fetch_headers` | `test_dialhelper` §6 (fake peer: continuation, genesis-first refusal, earlier-point overlap, fork refusal) |
| 7 | A stop seen after the boot catch-up exits before the worker starts. | boot path in `main.c` | — (pending deploy `v`) |
| 8 | The UTXO checkpoint is written as `utxo.idx.tmp` and renamed over `utxo.idx`. | `utxo_store_sync` (`bitcoin_utxo_store.asm`) | `test_utxo_store` (pending deploy `v`) |
...
| `t` (wallet output types; unrelated) | Its boot got a genesis-first answer **again**, from two peers in a row. The linkage guard rolled the first reply back (961,640 headers had already been appended before the asm call returned), but `node_ibd_headers` reported `0` added for the second peer's identical reply, so the guard never looked and the boot reported `already current (total 1926674)`. Stopped by hand (clean). Lesson: with the mirror now current, the boot asks from the newest block; a peer a block behind does not know it, and a one-hash locator gives it nothing else to match — genesis answers are *common*, not rare. |
| `u` (boot header fetch rewritten in C: exponential locator, per-page linkage/overlap/fork checks, `hst_count` deltas, cap; the asm fetch no longer used at boot) | Clean boot 12:33: self-heal cut the mirror at its divergence, `header mirror` topped up from the archive, header sync current, catch-up wrote 0 blocks, RPC up, `headers.dat` exactly 965,034 records. Live. |
...
Fixes: the boot now exits as soon as a stop is seen after the catch-up (`[boot] shutdown requested during the catch-up -- exiting before the worker starts`, commit below) — deployed **after** the rebuild completes, since a restart mid-rebuild would risk the same window. The checkpoint writer now writes `utxo.idx.tmp`, fsyncs it and renames it over `utxo.idx` (the long-known "SIGKILL/checkpoint window" is closed; `test_utxo_store` pins it). Both of these are committed and will be deployed as `v` once the replay has reached the tip.
...
- Commits: `2a98daa` (linkage guard), `7370316` (cap, mirror top-up, self-heal v1, SIGTERM, this report), `c5868d7` (chain-aware self-heal), `c5d8fa5` and `29a34a6` (the two top-up crashes), `e9ec869` (the C header fetch, live as `u`).
