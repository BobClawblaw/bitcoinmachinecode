...
| 3 | The header mirror is topped up **from the archive** (the blocks' own headers) at boot before any peer is asked, and after the worker stores new blocks — the mirror can no longer lag. | `dl_header_mirror_topup` | `test_dialhelper` §5 |
| 4 | Boot self-heal: trailing empty `index.dat` records past the tip are trimmed, and `headers.dat`/`chainwork.dat` longer than the index are cut to it, with a log line. This is how the production node repaired this incident's leftovers on its next start — no hand editing of archive files. | `archive_trim_derived_tails` (`daemon/archive_verify.c`) | `test_archive_trim` |
| 5 | The catch-up's worker-wait loop honours SIGTERM: workers are signalled and reaped, the boot returns. | `dl_catchup` | structural pin |
...
- The permission layer blocked the assistant's attempt to `truncate` the two derived files by hand; the repair therefore shipped as the product's own boot self-heal (fix 4), which is the better outcome.
- The signer build (`o`) itself was unrelated; it is redeployed as `p` together with these fixes.
