...
| 4 | the same block reconstructed several times | Core tracks a block in flight per peer and will not fetch a block it already has | **FOUND 2026-09-16 while remeasuring row 1.** 7 of 73 heights were reconstructed more than once, costing **11 MB** of redundant `getblocktxn` in one morning. Height 967,238 is the clearest: reconstructed at 05:48 from one peer (7,010 tx, 93.5% overlap, 55 KB), then **four more times at 06:07:43/46/49/49 from four different peers**, 4,469 tx each at 9-16% overlap and ~1.3 MB apiece -- **5.4 MB for one block**. | four reconstructions inside six seconds is a de-duplication failure, not four honest arrivals: the fetch gate that gives one request per block across the legs (`daemon/inflight.c`, PR #150) does not appear to cover the compact-block path. A second signal in the same data is unexplained and should not be assumed benign: three reconstructions at 05:18:5x all report **4,225 tx** while being labelled heights 967,230, 967,231 and 967,231 -- either the same block is being relabelled, or the logged height is not the block's own. | measure first: confirm whether the repeats are the same block by HASH rather than by height, which the log does not currently print. If they are, the fix is the existing in-flight gate applied to compact blocks. The height-labelling question must be answered before anything is changed, because a wrong height in the log would also make row 4's own evidence unreliable. |
