...
| n | the parallel download takes Core's shape: the window scales and anchors to the connected tip, the window's tail evicts stallers (since 2026-09-24 the tail's clock restarts on every block its holder delivers, Core's m_stalling_since; it had been a budget for the whole 40-block chunk, and at ~60 MB per recent chunk it banned working peers -- bmc_osx 0a11830e), replacement only when a free peer exists. Peers downloading at once defaults to 8, Core's MAX_OUTBOUND_FULL_RELAY_CONNECTIONS (was 64, never measured); the ceiling stays 64 for a fatter link | #157, #178 |
