...
HIGH) — written while it was the one finding of that review still open.

**Status: CLOSED (`4efb0c4`).** This document is kept as the reasoning behind the design that was chosen; the sections below were written before the change and
describe why it had to be shaped this way — the fix is a change to the stack
representation, and the representation is part of an ABI the daemon's C
verifier and five test harnesses depend on.
