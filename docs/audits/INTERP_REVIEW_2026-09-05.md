...
- **A fresh clone cannot pass `make test`.** The `link-check` prerequisite resolves "who defines symbol X" with `nm` over `*.o` files that exist; in a fresh checkout the asm objects do not exist yet and it does not assemble them itself, so every rule with an asm `.o` prerequisite looks unsatisfied and the gate aborts before running a test (observed twice, in two scratch worktrees, one with the daemon pre-built). The gate was only green on a warm tree. **Fixed the same day**: the audit now reads an unbuilt object through the source its own make rule names (assembling `.asm` and compiling `.c` into its cache, each source once), and an unbuilt archive as the union of its members; a fresh clone passes `link-check` in 2.3 s.
...
- **"Test/wallet only" linkage claims in the IR-2 and IR-10 commit messages were wrong.** They came from grepping rule text, which hides sources behind `$(...)` variables. `nm daemon/bitcoind` shows `segwit_v0_sighash` (the C one), `p2wpkh_verify`, `p2wsh_verify_checksig` and `verify_p2pkh` all linked into the daemon; `check_sig_encoding`, `sv_checksig_asm`, `taproot_verify_input_asm` and `verify_multisig` are not. Every caller was fixed regardless, so no fix changes -- the record does. Linkage is established with `nm` on the binary from now on.
