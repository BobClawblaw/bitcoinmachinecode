...
| N6 | **FIXED (host + repo)**, and worse than reported | logrotate had **never run**: 3.21 refuses a config that is group-writable *or not root-owned*, and the tracked file is both in a checkout. Root now reads its own copy `/etc/bmc/logrotate-bmc.conf` (the tracked file is the documented source, installed deliberately with `install -o root`); the rotation runs as the service user (`su <service-user> <service-user>`); the logs are service-user-owned. | forced run: euid switched to 1000, 4.46 MB log rotated to `.1` (<service-user>:<service-user>), daemon kept appending; unit run through systemd clean |
...
| N7 (nasm -Werror) | **DONE** -- N7 fully closed | Ten nasm warning sites in seven files fixed; `NASMFLAGS` carries `-Werror`. Four were real: a 48-bit AND that assembled to a no-op, three CompactSize writers whose 9-byte branch was unreachable (`cmp r, 0xffffffff` sign-extends to -1), and **CHECKSEQUENCEVERIFY's disable-flag test matching bits 32-63** (`test rax, 0x80000000`) -- a 5-byte operand with bit 32 set and bit 31 clear became a NOP where Core enforces it: a consensus false-accept on a valid script, the very class §2/N2 warns about. | `tests/test_csv_disable_flag` (gated) fails on the pre-fix object and passes now; gate 319/319, 0 warnings |
| §6.9 | **DONE** | Randomized differential of the asm interpreter vs Core's `EvalScript` (`tests/fuzz_script_diff` + `validation/core_script_oracle`). 728 divergences in the first 20k cases, five root causes, all policy-flag semantics (see worklog 2026-09-02 12:40). Fixed; 1.62M cases over 4 seeds now match exactly (verdict, error code, final stack). | `tests/test_interp_core_vectors` (17 Core-captured vectors, gated); gate 320/320 |
| §6.9 (extended) | **DONE** | Whole-input differential with Core-made signatures (`tests/fuzz_verify_diff` + `validation/core_verify_oracle`): legacy, P2SH, witness v0 native/wrapped, taproot key path and script path incl. annex, CODESEPARATOR, CHECKSIGADD, OP_SUCCESS, two-leaf trees; mutations. 0 verdict mismatches ever; the policy arms NULLFAIL / LOW_S / STRICTENC-hashtype / CONST_SCRIPTCODE were missing and are now implemented, matching Core's error precedence. 63k whole-input + 400k EvalScript cases clean. | `tests/test_verify_core_vectors` (71 Core-signed spends, gated) |
