...
| N6 | **FIXED (host + repo)**, and worse than reported | logrotate had **never run**: 3.21 refuses a config that is group-writable *or not root-owned*, and the tracked file is both in a checkout. Root now reads its own copy `/etc/bmc/logrotate-bmc.conf` (the tracked file is the documented source, installed deliberately with `install -o root`); the rotation runs as the service user (`su <service-user> <service-user>`); the logs are owned by it. | forced run: euid switched to 1000, 4.46 MB log rotated to `.1` (owned by the service account), daemon kept appending; unit run through systemd clean |
