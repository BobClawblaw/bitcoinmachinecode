...
| `coinstatsindex` at **any height** | one record per block; `gettxoutsetinfo <hash_type> <hash_or_height>` answers from it, with `block_info` as the difference from the previous block | the same since 2026-09-08: the live index writes a 1 KB row per applied block (`coinstats_hist.dat`) and `bmc_build_coinstats_hist` fills the rows from genesis from the archive alone into a separate base file the node verifies at boot and rebuilds itself when it is missing or broken (`bmc.coinstatshistrepair`, 2026-09-08); Core's output shape and error texts; `total_unspendable_amount` is sum(subsidy) - total_amount, an identity of Core's accounting verified at 800,000, 963,967 and 966,097 | **DONE** |
...
| REST interface | `-rest`: fourteen `/rest/` routes on the RPC port, unauthenticated, `.json`/`.hex`/`.bin` | the same (`rest=1`, 2026-09-08): every route from the node's RPC handlers, Core's status codes and error texts; `mempool_sequence=true` answered from the mempool sequence counter (2026-09-19; refused before) | **DONE** |
| ZMQ `hashblock`/`hashtx`/`rawblock`/`rawtx`/`sequence` | | same; `sequence` since 2026-09-19 (refused before; divergences in CORE_DIVERGENCES.md) | **DONE** |
