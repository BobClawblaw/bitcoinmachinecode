...
| Taproot **script-path** PSBT signing and finalization (`PSBT_IN_TAP_SCRIPT_SIG` 0x14, `PSBT_IN_TAP_LEAF_SCRIPT` 0x15, control blocks) | signed and finalized | **finalization DONE** 2026-09-06: `finalizepsbt` builds `[sig, leaf script, control block]` from 0x14/0x15, matching the leaf by its tagged hash and picking the cheapest control block when a wallet signed several leaves. Verified byte-for-byte against a spend Core signed and finalized on regtest (`tests/psbt_tapscript_vec.h`, descriptor `tr(NUMS,{pk(A),pk(B)})` so only the script path can spend; `testmempoolaccept` allows Core's tx). **SIGNING is still absent** — this node cannot produce a 0x14 signature of its own | **PARTIAL** |
