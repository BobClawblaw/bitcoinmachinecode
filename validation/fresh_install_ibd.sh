...
if [ "$RESUME" = 1 ]; then
    [ -x src/asm/daemon/bmcbitcoind ] || { ph "FAIL resume: no build at src/asm/daemon/bmcbitcoind"; echo FAIL > RESULT; exit 2; }
...
t0=$(date +%s); ( cd src/asm && make -s daemon/bmcbitcoind daemon/bmc_cli ) > build.log 2>&1 || { ph "FAIL build (see build.log)"; echo FAIL > RESULT; exit 1; }
...
ph "START daemon"
if [ "$RESUME" = 1 ]; then setsid nohup nice -n 10 ionice -c3 src/asm/daemon/bmcbitcoind serve "$DEST/data" >> console.log 2>&1 < /dev/null &
else setsid nohup nice -n 10 ionice -c3 src/asm/daemon/bmcbitcoind serve "$DEST/data" > console.log 2>&1 < /dev/null & fi
