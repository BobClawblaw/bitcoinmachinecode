...
```sh
cd asm
make daemon/bmcbitcoind    # the node daemon (asm/daemon/bmcbitcoind; was bitcoind until 2026-09-07)
...
2. **Run the daemon.**
   ...
   ```sh
   asm/daemon/bmcbitcoind serve /path/to/datadir [port] [nwant] [workers]
   ```
...
ExecStart=/path/to/repo/asm/daemon/bmcbitcoind serve /path/to/datadir
