 ...
 * macOS PORT NOTE (2026-09-09): this file is byte-identical upstream, but on
 * arm64 macOS the CONCURRENT gets test (8 threads x cold caches) fails while
 * single-threaded mm lookups are byte-correct; the asm/twin fallback passes
*  everything including concurrently. Root cause not yet pinned (suspect
 * __thread TLS or MAP_SHARED codegen difference under clang 21); until it
 * is, the fast path defaults OFF on __APPLE__ and stays available via
 * BMC_LSM_MMAP=1 for single-threaded tooling. The correctness anchor is the
 * twin lookup path, which is what production-equivalent runs use here. */
static __thread int g_enabled = -1;
static int mm_enabled(void){
    if (g_enabled < 0) {
#ifdef __APPLE__
        g_enabled = 0;               /* macOS: fallback is the default */
        const char *e = getenv("BMC_LSM_MMAP");
        if (e && e[0] == '1') g_enabled = 1;
#else
        const char *e = getenv("BMC_LSM_MMAP");
